...
for env in "${ambientes[@]}"; do
  if [[ $($GetPrj $env-$proj | awk '{print $2}' | tail -n1) == "Active" ]] &> /dev/null; then 
    ...
    kubectl get secret login-git -n dev-curupira-matas -o yaml | sed "s/namespace: .*/namespace: $env-$proj/" | oc apply -f -
    kubectl get secret quay-builder -n dev-curupira-matas -o yaml | sed "s/namespace: .*/namespace: $env-$proj/" | oc apply -f -
    # Linka o secret quay-builder ao usuário builder para realizar o pull das imagens
    oc secrets link builder quay-builder -n $env-$proj      
fi
done
...
for volume in "{$ambientes[@]}"; do
  if [[ $CriaVolume == 'Y' || $CriaVolume == 'y' ]]; then
    ...
    sed -i "s/NOME/$NomeVolume/" /root/pv.yml
    sed -i "s/NAMESPACE/$env-$proj/" /root/pv.yml
    sed -i "s/TAMANHO/$TamanhoVolume/" /root/pv.yml
    oc apply -f pv.yml
  fi
done
